...
var NEW_ARM_HEIGHT = 2.0;
var NEW_ARM_WIDTH  = 0.5;
     ...
     window.addEventListener("keydown", function(e){
         switch( e.keyCode ) {
            ...
            case 65:	// a - snýr neðri armi
			          theta[1] = Math.min(70, theta[1]+5);
                break;
            case 83:	// s - snýr neðri armi
			          theta[1] = Math.max(-70, theta[1]-5);
                break;
            case 81:	// q - snýr efri armi
			          theta[2] = Math.min(160, theta[2]+5);
                break;
            case 87:	// w - snýr efri armi
			          theta[2] = Math.max(-160, theta[2]-5);
                break;
            case 69: // e 
                theta[3] = Math.min(160, theta[3]+5);
                break;
            case 82:	// r
			          theta[3] = Math.max(-160, theta[3]-5);
                break;
         }
     }  );  
...
function newArm() {
  var s = scalem(NEW_ARM_WIDTH, NEW_ARM_HEIGHT, NEW_ARM_WIDTH);
  var instanceMatrix = mult(translate( 0.0, 0.5 * NEW_ARM_HEIGHT, 0.0 ),s);    
  var t = mult(modelViewMatrix, instanceMatrix);
  gl.uniformMatrix4fv( modelViewMatrixLoc,  false, flatten(t) );
  gl.drawArrays( gl.TRIANGLES, 0, NumVertices );
}
...
var render = function() {
    ...
    modelViewMatrix  = mult(modelViewMatrix, translate(0.0, LOWER_ARM_HEIGHT, 0.0));
    modelViewMatrix  = mult(modelViewMatrix, rotateZ( theta[3] ) );
    newArm();
