...
var render = function() {
    ...
    for (var i = 0; i < 2; i++) {
      if (direction[i] == 1) {
          theta[i] = Math.min(maxDegree[i], theta[i] + speed[i]);
      } else {
          theta[i] = Math.max(-maxDegree[i], theta[i] - speed[i]);
      }
  
      if (theta[i] >= 90 || theta[i] <= -89) {
          direction[i] *= -1;
      }
  }
    ...
    modelViewMatrix = mult(modelViewMatrix, rotateZ( theta[0] ));
    lowerArm();

    modelViewMatrix  = mult(modelViewMatrix, translate(0.0, LOWER_ARM_HEIGHT, 0.0));
    modelViewMatrix  = mult(modelViewMatrix, rotateZ( theta[1] ) );
    upperArm();
